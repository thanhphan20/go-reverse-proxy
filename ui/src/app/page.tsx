  ...
  const handleTestRoute = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}${testPath.startsWith("/") ? testPath : "/" + testPath}`
      );
        ...
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Live dashboard · real-time via SSE
            </p>
          </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur rounded-2xl border border-gray-700/50 p-6 flex flex-col gap-4">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-gray-700">
              {Object.entries(routes).map(([path, target]) => (
                <div
                  key={path}
                  className="group flex items-center justify-between bg-gray-900/60 px-3 py-2.5 rounded-xl border border-gray-700/40 hover:border-indigo-500/40 transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="font-mono text-indigo-300 text-sm font-semibold shrink-0">
                      {path}
                    </span>
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                    <button
                      onClick={() => handleEditRoute(path, target)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <p className="text-sm text-gray-500">
                Fire a test request through the proxy and inspect the response.
              </p>
